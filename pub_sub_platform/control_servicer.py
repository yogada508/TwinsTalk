...
class ControlServicer(node_pb2_grpc.ControlServicer):
    ...
    def Register(self, request: node_pb2.NodeInfo, context) -> node_pb2.Empty:
        ...
        print('Register: NodeID={}, NodeName={}, NodeDomain={}'.format(request.node_id, request.node_name, request.node_domain))

        try:
            self.db.insert_node(request.node_id, request.node_name, request.node_domain, self.get_current_time())
    ...
    def AddTopic(self, request: node_pb2.Topic, context) -> node_pb2.Empty:
        ...
        try:
            topic=self.db.get_topic_info(request.topic_name,request.topic_type)
            if len (topic)==0:
                print('AddTopic: TopicName={}, TopicType={},TopicMode={}, TopicIP={}, TopicPort={}, NodeID={}'.format(
                    request.topic_name, request.topic_type, request.mode,request.ip,
                    request.port, request.node_id))
                self.db.insert_topic(request.topic_name, request.topic_type, request.node_id, request.node_domain, self.get_current_time(),request.mode,request.ip, request.port)
            else:
                self.db.update_topic(request.topic_name, request.topic_type, request.node_id, request.node_domain, self.get_current_time(),request.mode,request.ip, request.port)
        except Exception as e:
            context.set_code(grpc.StatusCode.ALREADY_EXISTS)
            context.set_details(str(e))
            return node_pb2.Empty()
    ...
    def DeleteTopic(self, request: node_pb2.Topic, context) -> node_pb2.Empty:
        ...
        print('DeleteTopic: TopicName={}, TopicType={}, NodeID={}'.format(request.topic_name, request.topic_type, request.node_id))

        try:
            self.db.delete_topic(request.topic_name, request.topic_type, request.node_id)
            self.db.delete_connection_by_topic(request.topic_name, request.topic_type)
    ...
    def UpdateTopicStatus(self, request: node_pb2.TopicStatus, context) -> node_pb2.Empty:
        ...
        print('Update: TopicName={}, NodeID={}, ConnectedNodes={}'.format(request.topic_name, request.node_id, request.connected_nodes))
        ...
        try:
            ...
            for connected_node in request.connected_nodes:
                self.db.update_connection_status(request.topic_name, request.node_id, connected_node, 1)
            ...
            for connection_info in connections_info:
                if connection_info['PubTopicName'] == request.topic_name:
                    if connection_info['SubNodeID'] not in request.connected_nodes:
                        self.db.update_connection_status(connection_info['PubTopicName'], connection_info['PubNodeID'], connection_info['SubNodeID'], 0)
    ...
    def AddSubscribeTopic(self, request: node_pb2.SubscribeTopic, context) -> node_pb2.Empty:
        print('AddSubscribeTopic: TopicName={}, TopicType={}, NodeID={}'.format(request.topic_name, request.topic_type, request.node_id))
        
        try:

            topic=self.db.get_subscribe_topics_info(request.topic_name,request.topic_type)
            if len(topic)==0:
                self.db.insert_subscribe_topic(request.topic_name, request.topic_type, request.node_id, request.node_domain, self.get_current_time(), request.mode,request.ip,request.port)
            else:
                self.db.update_subscribe_topic(request.topic_name, request.topic_type, request.node_id, request.node_domain, self.get_current_time(), request.mode,request.ip,request.port)
        except Exception as e:
            context.set_code(grpc.StatusCode.ALREADY_EXISTS)
            context.set_details(str(e))
            return node_pb2.Empty()
    ...
    def DeleteSubscribeTopic(self, request: node_pb2.SubscribeTopicInfo, context) -> node_pb2.Empty:
        print("DeleteSubscribeTopic: TopicName={}, TopicType={}".format(request.topic_name, request.topic_type))

        try:
            self.db.delete_subscribe_topic(request.topic_name, request.topic_type)
            self.db.delete_connection_by_subscribe_topic(request.topic_name, request.topic_type)
            return node_pb2.Empty()
    ...
    def UpdateSubscribeTopicStatus(self, request: node_pb2.SubscribeTopicStatus, context) -> node_pb2.Empty:
        ...
        try:
            self.db.update_subscribe_topic_time(request.topic_name, request.node_id, self.get_current_time())
        except Exception as e:
            context.set_code(grpc.StatusCode.NOT_FOUND)
            context.set_details(str(e))
            return node_pb2.Empty()
    ...
    def CreateResponseConnection(self, connections_info):
        ...
        for connection_info in connections_info:
            ...
            topic_info={}
            pub_topic_info = self.db.get_topic_info(connection_info['PubTopicName'], connection_info['TopicType'])
            sub_topic_info = self.db.get_subscribe_topics_info(connection_info['SubTopicName'], connection_info['TopicType'])
            ...
            if topic_info:
                ...
                print("response connections_info",response_topic_info,topic_info)
                print("-------------------------------------------------")
    ...
    def GetConnection(self, request: node_pb2.RequestConnection, context) -> node_pb2.ResponseConnection:
        ...
        print('GetConnection: NodeID={} isSubscriber={}'.format(request.node_id,request.isSubscriber))
