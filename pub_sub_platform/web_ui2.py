...
server="http://140.113.193.15:5000"
app = Flask(__name__)
Pub_Path=defaultdict(dict)
Sub_Path=defaultdict(dict)
Publication=defaultdict(dict)
Subscription=defaultdict(dict)
def time_format(sec):
...
def connection():
    ...
    for topic in topics:
        topic_class,topic_name=split_topic_name(topic[0])
        topics_type.add(topic[1])
        if(topic_name not in sub_topics_name):
            pub_topics_name.add(topic_name)
        if(topic_class not in sub_nodes):
            pub_nodes.add(topic_class)
    ...
    for subscribe_topic in subscribe_topics:
        topic_class,topic_name=split_topic_name(subscribe_topic[1])
        if(topic_name not in sub_topics_name):
            sub_topics_name.add(topic_name)
        if(topic_class not in sub_nodes):
            sub_nodes.add(topic_class)
...
def add_connection():
    ...
    if request.method == "POST":
        ...
        db.insert_rule(request.values.get('PubTopicName'),
